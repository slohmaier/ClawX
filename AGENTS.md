...
- **Accessibility gate**: Any user-visible UI change must also (a) pass `pnpm run lint` without new `eslint-plugin-jsx-a11y` violations and (b) ship an axe test — a new spec under `tests/unit/a11y/` for a component, or an addition to `tests/e2e/a11y.spec.ts` for a route. New `aria-label` / `aria-describedby` strings must land in all four locales (`src/i18n/locales/{en,zh,ja,ru}/`). Status matrix: `ACCESSIBILITY.md`. Keyboard shortcuts source-of-truth: `docs/keyboard-shortcuts.md`.
