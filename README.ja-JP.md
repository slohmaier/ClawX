...
### ♿ アクセシビリティ
ClawX はスクリーンリーダー（VoiceOver / NVDA / JAWS / Orca）およびキーボードのみの操作を一級市民として扱い、WCAG 2.1 AA への準拠を目指しています。すべての PR は `eslint-plugin-jsx-a11y` による静的検査と axe ベースの単体・E2E テストで検証されます。現在の対応状況は [`ACCESSIBILITY.md`](ACCESSIBILITY.md)、キーボードショートカット一覧は [`docs/keyboard-shortcuts.md`](docs/keyboard-shortcuts.md) を参照してください。
