...
### ♿ Доступность
ClawX разрабатывается с ориентиром на WCAG 2.1 AA с первоклассной поддержкой программ чтения с экрана (VoiceOver, NVDA, JAWS, Orca) и работы только с клавиатуры. Каждый PR проверяется статическим линтером `eslint-plugin-jsx-a11y`, а также axe-based модульными и end-to-end тестами. Актуальный статус см. в [`ACCESSIBILITY.md`](ACCESSIBILITY.md), полный список горячих клавиш — в [`docs/keyboard-shortcuts.md`](docs/keyboard-shortcuts.md).
